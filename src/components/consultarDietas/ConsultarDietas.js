...
import { getDietasId } from '../../actions/admin'
...
export const ConsultarDietas = () => {
   ...
   const handdleGetRoutines = async () => {
    ...
    const data =   await dispatch(getDietasId(uid));
    ...
    table.innerHTML = `<table>${data_html}</table><p>Total de Rutinas: ${data.length}</p>`;
     ...
     return (
       <>
           ...
           <button className='btn btn-success btnAdmin '  onClick={handdleGetRoutines}>OBTENER DIETAS</button>
